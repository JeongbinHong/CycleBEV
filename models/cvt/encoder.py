...
class CrossViewAttention(nn.Module):
    ...
    def forward(
        ...
        cam = I_inv @ pixel_flat                                                # b n 3 (h w) = [b n 3 3] @ [1 1 3 (hw)]
        cam = F.pad(cam, (0, 0, 0, 1, 0, 0, 0, 0), value=1)                     # b n 4 (h w)
        d = E_inv @ cam                                                         # b n 4 (h w) = [b n 4 4] @ [b n 4 (hw)]
        ...
        val_flat = self.feature_linear(feature_flat) # pi                         # (b n) d h w
                        # x : 'learned_features(= query grid)' from BEVEmbedding  
...
class Encoder(nn.Module):
    def __init__(
        ...
        nbl = cfg['CVT']['encoder']['num_bottleneck_layers']
        ...
        for feat_shape in img_feat_shapes:
            _, feat_dim, feat_height, feat_width = self.down(torch.zeros(feat_shape)).shape

            cva = CrossViewAttention(feat_dim, feat_height, feat_width, dim, cfg, **cross_view)
        ...
        # self.layers = nn.ModuleList([nn.Sequential(*[ResNetBottleNeck(dim) for _ in range(nbl)]) 
        #                              for _ in range(args.num_encoder_layers)])
        # self.norm = nn.BatchNorm2d(dim)
    def forward(self, batch, features):

        if batch['extrinsics_inv'].ndim == 5:
