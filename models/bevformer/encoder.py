...
from models.bevformer.deformable_transformer import DeformableTransformerEncoderLayer
...
class BEVFormerEncoder(nn.Module):
    def __init__(self, cfg, query_map_size):
        super().__init__()

        self.cfg = cfg
        self.h_dim = cfg['BEVFormer']['encoder']['dim']
        self.n_cam = cfg['image']['n_cam']
        self.n_lvl = len(cfg['BEVFormer']['encoder']['feat_levels'])
        self.z_candi = cfg['BEVFormer']['encoder']['z_candi']
        # self.h, self.w = cfg['bev']['h'] // int(scale), cfg['bev']['w'] // int(scale)
        self.h, self.w = query_map_size

        if (self.h_dim == 128):
            d_ffn, n_heads = 512, 4
        elif (self.h_dim == 256):
            d_ffn, n_heads = 1024, 8
        else:
            sys.exit(f'h_dim {self.h_dim} is not supported...')
        ...
        encoder = DeformableTransformerEncoderLayer(cfg=cfg, query_map_size=query_map_size, d_model=self.h_dim, d_ffn=d_ffn, dropout=0.1,
                                                    activation="relu", n_levels=self.n_lvl, n_heads=n_heads, n_points=4, mixed_precision=False)

        # Transformer Encoder
        self.DeformAttnEnc = nn.ModuleList([copy.deepcopy(encoder) for _ in range(cfg['BEVFormer']['encoder']['repeat'])])
    ...
    def forward(self, features, intrinsics, extrinsic):
        '''
        ...
        b = intrinsics.size(0)
        queries = self.bev_queries_init.weight[None].repeat(b, 1, 1)  # b (h w) d
        ...
        return rearrange(queries, 'b (h w) d -> b d h w', h=self.h, w=self.w)
