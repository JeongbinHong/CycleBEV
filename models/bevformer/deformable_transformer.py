...
from models.bevformer.ops.functions import MSDeformAttnFunction
from models.bevformer.ops.modules import MSDeformAttn, MSDeformAttn3D
...
class TemporalSelfAttention(nn.Module):
    def __init__(self, cfg, query_map_size=(200,200), d_model=256, n_levels=1, n_heads=8, n_points=4, mixed_precision=False):
        super().__init__()
...
class SpatialCrossAttention(nn.Module):
    def __init__(self, cfg, d_model=256, n_levels=1, n_heads=8, n_points=4, mixed_precision=False, query_map_size=(200,200)):
        super().__init__()
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self, cfg, query_map_size=(200,200), d_model=256, d_ffn=1024, dropout=0.1, activation="relu",
        ...
        # self attention
        self.self_attn = TemporalSelfAttention(cfg, query_map_size, d_model, 1, n_heads, n_points, mixed_precision)
        self.dropout0 = nn.Dropout(dropout)
        self.norm0 = nn.LayerNorm(d_model)

        # cross attention
        self.cross_attn = SpatialCrossAttention(cfg, d_model, n_levels, n_heads, n_points, mixed_precision, query_map_size)
