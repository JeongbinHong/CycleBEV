...
class NuScenes:
    ...
    def __init__(self,
        ...
        self.attribute = self.__load_table__('attribute') # 안 씀
        self.visibility = self.__load_table__('visibility') # 안 씀
        self.instance = self.__load_table__('instance') # 안 씀
        self.sensor = self.__load_table__('sensor') # 안 씀
        self.calibrated_sensor = self.__load_table__('calibrated_sensor') # 안 씀
        self.ego_pose = self.__load_table__('ego_pose') # 안 씀
...
def interpolate_boxes_to_times(boxes, box_timestamps_relative, lidar_tokens, relative_times_interp):
    ...
    # # Minor manipulation, update, 240131
    # if (box_timestamps_relative[0] > relative_times_interp[0]):
    #     box_timestamps_relative[0] = relative_times_interp[0]
    #
    # if (box_timestamps_relative[-1] < relative_times_interp[-1]):
    #     box_timestamps_relative[-1] = relative_times_interp[-1]

    assert(min(box_timestamps_relative) <= min(relative_times_interp) < max(relative_times_interp) < max(box_timestamps_relative))

    interpolated, lidar_tokens_sorted = [], []
    ...
    for interp_time, sort_index in zip(relative_times_interp, sort_indices):
        if sort_index == 0 or sort_index == n_boxes:
            # update, 240131
            # interpolated.append(boxes[sort_index])
